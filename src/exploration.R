...
boroughs <- merge(boroughs, ascData, by.x = "GSS_CODE", by.y = "ONS_Code")


boroughs$asc_per_pop <- round(boroughs$asc_total/boroughs$GLA_Population_Estimate_2015,1)
boroughs$mental_health_spend_per_pop <- round((boroughs$X65_and_over_Mental_Health_Support+boroughs$X18_to_64_Mental_Health_Support)/boroughs$GLA_Population_Estimate_2015,1)
